...
// Map data from therapistsStore -> UsersPage shape
function fromStoreTherapist(t) {
  const id = normalizeDigits(t?.id) || "";
  return normalizeTherapistRecord({
    id: id || uid(),
    idNumber: id,
    fullName: normalizeString(t?.name) || id,
    active: t?.active !== false,
  });
}

// Map UsersPage draft -> therapistsStore expected shape
function toStoreTherapist(item) {
  const id = normalizeDigits(item?.idNumber) || normalizeDigits(item?.id) || "";
  const name = normalizeString(item?.fullName) || id;
...
export default function UsersPage({ handleSyncAllTherapistsToMedplum }) {
  ...
  // ✅ This is the missing piece: load from Supabase via therapistsStore
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        const mapped = (Array.isArray(list) ? list : []).map(fromStoreTherapist);
  ...
  async function upsertItem(next) {
    ...
    // Best-effort cloud write (does not affect Medplum)
    try {
      await upsertTherapist(toStoreTherapist(normalizedNext));
    } catch (e) {
      console.warn("[UsersPage] upsertTherapist failed:", e);
    }
  }
  ...
  async function onDelete(id) {
    ...
    try {
      ...
      console.warn("[UsersPage] deleteTherapist failed:", e);
    }
  }
