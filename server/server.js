...
app.post("/api/ai/improve-visit", async (req, res) => {
  try {
    ...
    const system = [
      ...
      "- Use only information explicitly present in the source note. Do not infer or extrapolate.",
      "- Do NOT add observations not stated in the source: no gait mechanics, no ROM values,",
      "  no ambulation descriptions, no tolerance or response statements, no timeline references",
      "  (e.g. 'since last session', 'over the past week') unless the source explicitly states them.",
      "- BANNED generic phrases: 'demonstrates improvement', 'shows progress', 'doing well',",
      "  'good session', 'patient tolerated well', 'continue as before', 'no concerns noted',",
      "  'patient was able to', 'ambulated independently', 'participated actively'.",
      "  Replace only with what the source actually states.",
      "- Clinical phrasing: write in formal third-person clinical style.",
      "  Use 'Patient reports...' for subjective complaints, not 'has pain' or 'is in pain'.",
      "  Use passive constructions for interventions: 'Exercises were performed', 'Manual therapy was applied',",
      "  not 'did exercises' or 'worked on'.",
      "  Use 'Treatment was directed toward...' not 'worked on' or 'focused on'.",
      "  Keep sentences short and precise.",
      "- Subjective: what the patient explicitly reported — pain location, intensity",
      "  (use numeric scale only if mentioned in the source), functional complaints.",
      "- Objective: only what the therapist explicitly documented — specific exercises",
      "  with sets/reps/resistance if stated, measurable clinical findings if stated.",
      "- Assessment: which impairment is being addressed. Do not assert progression or regression",
      "  unless the source directly supports it. Keep neutral if source is vague.",
      "- Plan: concrete next steps drawn from the source only. Do not invent a plan.",
      "- If a section has insufficient source information, write one short sentence or omit it.",
